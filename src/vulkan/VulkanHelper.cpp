...
namespace VulkanHelper {
    ...
    void endSingleTimeCommands(const std::shared_ptr<VulkanContext>& ctx, VkCommandBuffer commandBuffer) {
        ...
        // Submit on whichever queue the command pool was created against:
        // graphics-capable when present, otherwise compute.
        VkQueue queue = ctx->graphicsQueue != VK_NULL_HANDLE ? ctx->graphicsQueue : ctx->computeQueue;
